...
import type { PokemonWithJapaneseName, PokemonListWithJapaneseNames } from '../api/pokemonWithJapaneseName';
import { INITIAL_POKEMON_LIST_LIMIT } from '../config';
import PokemonCard from '../components/PokemonCard';
import Skeleton from 'react-loading-skeleton';
import 'react-loading-skeleton/dist/skeleton.css';

// PokemonCard コンポーネントは別ファイルに定義されていると仮定
const PokemonList: React.FC = () => {
  const {
    ...
    initialPageParam: 0,
    getNextPageParam: (lastPage: PokemonListWithJapaneseNames, pages: PokemonListWithJapaneseNames[]) => {
      if (lastPage.next) {
